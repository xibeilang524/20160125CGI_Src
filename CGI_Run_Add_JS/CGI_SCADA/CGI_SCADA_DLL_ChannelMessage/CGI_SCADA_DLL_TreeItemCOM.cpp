...
CGI_SCADA_DLL_TreeItemCOM::CGI_SCADA_DLL_TreeItemCOM(int nComNumber_,QTreeWidgetItem *parentItem, const QStringList &strings, int type, QObject *parent) :
    CGI_SCADA_DLL_TreeItemBase(parentItem,strings,type,parent),
    m_nComNumber(nComNumber_),
...
CGI_SCADA_DLL_TreeItemCOM::~CGI_SCADA_DLL_TreeItemCOM()
{
...
bool CGI_SCADA_DLL_TreeItemCOM::SetChannelFile(QString strFileName_)
{
    ...
    for (int nDeviceIndex = 0; nDeviceIndex < DeviceNodeList.count(); ++nDeviceIndex)
    {
        ...
        CGI_SCADA_DLL_TreeItemDevice *pDeviceItem = new CGI_SCADA_DLL_TreeItemDevice(m_nProtocolType,device,this,QStringList()<<DeviceElem.attribute("Name"),TreeItemType_ChanneM_COM_Device,this);
...
int CGI_SCADA_DLL_TreeItemCOM::GetComNumber() const
{
    return m_nComNumber;
}

bool CGI_SCADA_DLL_TreeItemCOM::SaveAction()
{
    QDomDocument SCADApro;
    QDomElement ProRoot = SCADApro.createElement(tr("Root"));
    SCADApro.appendChild(ProRoot);

//    m_pAttributeItem->SaveAction(SCADApro,ProRoot);
...
bool CGI_SCADA_DLL_TreeItemCOM::SetInitTagMap(QMap<QString, QStringList> &TagMap_)
{
...
bool CGI_SCADA_DLL_TreeItemCOM::SetBuildTagMap(const QMap<QString, QStringList> &TagMap_)
{
    switch (m_nProtocolType) {
    case ProtocolType_Transform:
        for (int nDeviceIndex = 0; nDeviceIndex < this->childCount(); ++nDeviceIndex)
...
QWidget *CGI_SCADA_DLL_TreeItemCOM::GetWidget(QTreeWidgetItem */*pItem_*/)
{
...
void CGI_SCADA_DLL_TreeItemCOM::slot_AddDevice()
{
    ...
    if (ret)
    {
        ...
        CGI_SCADA_DLL_TreeItemDevice *pDeviceItem = new CGI_SCADA_DLL_TreeItemDevice(m_nProtocolType,device,this,QStringList()<<strDeviceName,TreeItemType_ChanneM_COM_Device,this);
        connect(pDeviceItem,SIGNAL(signal_ShowMessageDebug(int,QString)),this,SIGNAL(signal_ShowMessageDebug(int,QString)));
        connect(this,SIGNAL(signal_ProtocolTypeChange(int)),pDeviceItem,SIGNAL(signal_ProtocolTypeChange(int)));
        this->addChild(pDeviceItem);
    }else
...
void CGI_SCADA_DLL_TreeItemCOM::slot_DeleteChannel()
{
    qDebug()<<__func__;
...
void CGI_SCADA_DLL_TreeItemCOM::slot_ProtocolTypeChange(int nProtocolType_)
{
    m_nProtocolType = (ProtocolType)nProtocolType_;
    qDebug()<<__func__<<__LINE__<<m_nProtocolType;
}

void CGI_SCADA_DLL_TreeItemCOM::slot_SetEnable(bool enable)
{
