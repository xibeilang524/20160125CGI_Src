 ...
 * \date   2015/5/26
 */
void CProtocolDeliverIEC_104::ProcessASDU101Type_C_CI_NA_1(ASDU101 &/*asdu*/)
{
...
//        qDebug() <<"tagIterator .key()="<<tagIterator.key()<<"nPointnumber="<<nPointNumber;
        ++nPointNumber;
...
/*!
 * \brief  功能概述 合成并发送标度化遥测值（带品质描述、不带时标）帧
 * \param  参数描述 nSendNo是发送序号，nReceiveNo是接收序号，nCommonAddress是公共地址
 * \return 返回值描述 如果成功则返回true，否则返回false
 * \author zzy
 * \date   2015/5/26
...
////////不带时标
    ...
    *pCur++ = 0; //公共地址高位
    ...
    for (PointIterator = YX_PointMap.begin(); PointIterator != YX_PointMap.end(); ++PointIterator)
    {
        if(PointIterator.value()->m_strRTDBName == pPointBase_->m_strRTDBName)
        {
            if (PointIterator.value()->GetValue()->GetVarValue() == VarSet_)
            {
