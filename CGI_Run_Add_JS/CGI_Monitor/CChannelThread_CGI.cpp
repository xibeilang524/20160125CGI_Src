...
CChannelThread_CGI::CChannelThread_CGI(int nChannelNumber_, ChannelType nChannelType_, QObject *parent) :
    QThread(parent),
...
CChannelThread_CGI::CChannelThread_CGI(int nChannelNumber_, ChannelType nChannelType_, int nComOrCanNumber_, QObject *parent) :
    QThread(parent),
    m_nChannelNumber(nChannelNumber_),
    m_nChannelType(nChannelType_),
...
#endif
        break;
    case ChannelType_DeliverMonitor104:
#if defined(TIAOSHI)
        m_LibName = QString("Protocol/MonitorIEC-104_d");
#else
        m_LibName = QString("Protocol/MonitorIEC-104");
#endif
    default:
        break;
    }
...
void CChannelThread_CGI::run()
{
    ...
    if (mylib.load())
    {
        ...
        typedef CProtocolI * (DLLAPI_CreateDriver)( );//初始化协议
        ...
        if (pCreateDriver != NULL)
        {
            ...
            m_pProtocolI = pCreateDriver();///< 库中导出类的初始化
            ...
            if (ChannelType_Monitor104 == m_nChannelType)
            {
                ...
                m_pProtocolI->OnCreateChannel(m_strMonitorIPAddress,&g_RealTimeDB);
            }else if (ChannelType_DeliverMonitor104 == m_nChannelType)
            {
                CMyFunction::pMonitorProtocol = m_pProtocolI;
                m_pProtocolI->SetPRTMap(&g_PRTMap);
                m_pProtocolI->OnCreateChannel(m_strMonitorIPAddress,&g_RealTimeDB);
...
//                m_pProtocolI->SetPRTMap(&g_PRTMap);
//                m_pProtocolI->OnCreateChannel(m_ChannelElement,(ChannelType)m_nChannelType,m_nComOrCanNumber,&g_RealTimeDB);
            }
        }else
        {
