...
//////////////////////////////////////////////////////////////////////////
//设置字节的位
#define SET_BIT(nData,nBit) ((nData)|(1<<(nBit)))
//复字节的位
#define RESET_BIT(nData,nBit) ((nData)&(~(1<<(nBit))))
//取字节的位
#define GET_BIT(nData,nBit)  ((nData)>>(nBit) &1)
//一个字的低N位都是0，其余位都是1
#define LOW_ZEROES(n) (-1<<n) //如果N的值不大于int类型的宽度
//一个字的低N位都是1，其余位都是0
#define LOW_ONES(n) (~LOW_ZEROES(n))
//一个字的低offset位都是1，接着的width位都是0 ，其余位都是1
#define MID_ZEROES(width,offset) \
    (LOW_ZEROES(width+offset)|LOW_ONES(offset))
//一个字的低offset位都是0，接着的width位都是1，其余位都是0
#define MID_ONES(width,offse) (~MID_ZEROES(width,offset))
//取从offset位开始的width位
#define GET_MID_BIT_VALUE(nValue,nStartBit,nBitNumber) \
    (((nValue)>>(nStartBit))&LOW_ONES(nBitNumber))
//循环左移位
#define CY_LEFT_SHIFT(nValue,nShift,nBit)\
    ((nValue<<nShift) | ((nValue>>(nBit-nShift))&LOW_ONES(nShift))) & LOW_ONES(nBit)
//定义ASDU101类型标识
enum ASDU101_TYPE
{                   // 报文语义   其它说明
...
///监控方向的过程信息类型标识(上行信息)
    ...
    M_RE_NA_1=130,  ///// 充电桩业务数据8
    M_MD_NA_1=132,  ///// 测量值，标度化值，长度大于2字节
    M_JC_NA_1=134,  ///// 交流充电桩实时监测数据项
///控制方向的过程信息类型标识(下行信息)
...
/// 在控制方向的系统信息(下行信息)
    C_IC_NA_1=100,	///// 站总召唤命令     带不同的限定词可以用于组召唤
    C_CI_NA_1=101,	///// 计数量总召命令 累计量召唤命令  带不同的限定词可以用于组召唤
...
public:
    ...
    int GetCommonAddress();//应用服务数据单元公共地 址
