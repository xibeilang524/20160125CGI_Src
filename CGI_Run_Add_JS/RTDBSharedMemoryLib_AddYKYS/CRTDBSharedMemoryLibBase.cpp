...
{
    qDebug()<<__func__<<__LINE__<<__FILE__<<3<<nID_<<varValue_;
...
//        qDebug()<<"nID_ ==== "<<nID_;
        if(pSharedMemoryTag)
        {
            ...
            m_SharedMemory.lock();
            int nType = *(int *)(to+nPianYi);
            qDebug()<<__func__<<__LINE__<<"m_isSucceed"<<m_isSucceed<<strRTDBName_<<"nType"<<nType<<QVariant::Bool<<QMetaType::Float;
            switch (nType) {
            case QVariant::Bool:
            {
                int bValue = *(int *)(to + nPianYi + 4);
            ...
            case QMetaType::Float:
            {
                int aaa = *(int *)(to+nPianYi+4);
                qDebug()<<__func__<<__LINE__<<aaa;
                float fValue = *(float *)(to + nPianYi + 4);
                QVariant var(fValue);
                sharedMemoryPoint.m_nVariant = var;
            }
